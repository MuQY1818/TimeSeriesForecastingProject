...
def probe_feature_set(df: pd.DataFrame, target_col: str):
    """
    SPECIALIST PROBE for Transformer.
    Evaluates a feature set's quality by testing it exclusively on the Transformer model.
    The primary score is the raw R^2 score from this model.
    """
    # 1. Prepare Data
    ...
    if X.empty or y.empty or len(X) < 20:
        ...
        return {"primary_score": 0.0, "r2_transformer": 0.0, "num_features": 0}

    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=False)

    if len(X_train) < 1 or len(X_test) < 1:
        print("  - ⚠️ Warning: Not enough data for train/test split. Returning poor score.")
        return {"primary_score": 0.0, "r2_transformer": 0.0, "num_features": X.shape[1]}

    # --- Specialist Model: Transformer ---
    ...
    # --- Score Logic ---
    primary_score = max(0.0, score) # Clip at 0, raw R2 can be negative

    return {
        "primary_score": primary_score,
        "r2_transformer": score,
...
class TLAFS_Algorithm:
    ...
    def get_plan_from_llm(self, context_prompt, iteration_num, max_iterations):
        ...
        base_prompt = f"""You are a Data Scientist RL agent. Your goal is to create a feature engineering plan to maximize the R^2 score.
...
"""
    ...
    def build_llm_context(self, probe_results, iteration_num):
        ...
        context = {
            ...
            "Current Transformer R2 Score": probe_results['primary_score'],
            "Historical Best Score": self.best_score,
            "Number of Features": probe_results.get('num_features', 0),
    ...
    def format_prompt_for_llm(self, context_dict, in_context_examples_str):
        ...
        prompt += "Your goal is to propose a feature engineering plan to maximize the 'Current Transformer R2 Score'.\n"
    ...
    def run(self):
        print(f"\n💡 Starting T-LAFS with Transformer Specialist Probe (RL Framework) ...\n")
        ...
        try:
            ...
            print(f"  - Initial baseline score (Transformer R²): {self.best_score:.4f} | Features: {probe_results.get('num_features', -1)}")
        except Exception as e:
            import traceback
        ...
        for i in range(self.n_iterations):
            print(f"\n----- ITERATION {i+1}/{self.n_iterations} (Probe: Transformer Specialist) -----")
            
            last_results = self.last_probe_results
            print(f"  - Current Score (Transformer R²): {last_results['primary_score']:.4f} | #Feats: {last_results.get('num_features', -1)} | Best Score: {self.best_score:.4f}")
            ...
            print(f"  - Probe results: Transformer R2 Score={new_score:.4f}, #Feats: {new_probe_results.get('num_features', -1)}")
        ...
        print("\n" + "="*80 + f"\n🏆 T-LAFS (Transformer Specialist Probe) Finished! 🏆")
...
def main():
    ...
    print("="*80)
    print(f"🚀 T-LAFS Experiment: Specialist Search for Transformer Model")
    print("="*80)
    ...
    if best_df is not None:
        probe_name_for_reporting = "SpecialistSearch_Transformer"
        print("\n" + "="*40)
        ...
        if final_metrics:
            ...
            print("="*60)
            print(f"Feature engineering search was conducted by: a SPECIALIST PROBE for the TRANSFORMER model.")
            if not USE_SAVED_PLAN:
                print(f"   - Best Transformer R² during search phase: {best_score_during_search:.4f}")
            print(f"\nBest feature plan discovered for this model:")
            ...
            print("powerful feature set, which was then validated against other models.")
            print("="*60)
