...
def create_static_features(df: pd.DataFrame, target_col: str = 'Target') -> pd.DataFrame:
    """
    创建静态特征
...
def train_pytorch_model(model, X_train, y_train, X_val, y_val, X_test, epochs=100, batch_size=32, lr=0.001, model_name='model'):
    ...
    # 初始化早停
    checkpoint_path = f'checkpoints/{model_name}_control_best.pth'
...
def evaluate_on_multiple_models(df: pd.DataFrame, target_col: str = 'Target') -> Dict[str, Any]:
    """在多个模型上评估数据"""
    print(f"\n开始评估多个模型，目标列: {target_col}")
    
    # 准备数据
    y = df[target_col].values
    X = df.drop(columns=['Date', target_col]).values # 使用所有其他列作为特征
    
    # 标准化特征
    scaler = StandardScaler()
    X = scaler.fit_transform(X)
    
    print(f"特征数据形状: X: {X.shape}, y: {y.shape}")
    ...
    for name, model in models.items():
        ...
        trained_model, history, y_pred = train_pytorch_model(
            model, X_train, y_train, X_val, y_val, X_test, model_name=name
        )
...
def plot_results(results: Dict[str, Any], experiment_dir: str):
    """绘制实验结果"""
    y_test = results.pop('test_data')
    
    for model_name, model_results in results.items():
        plot_predictions(
            y_true=y_test,
            y_pred=model_results['predictions'],
            title=f'{model_name} - 预测结果对比 (控制组)',
...
def main():
    ...
    experiment_dir = os.path.join('results', f'control_{timestamp}')
    os.makedirs(experiment_dir, exist_ok=True)
    print(f"创建实验目录: {experiment_dir}")
    
    # 创建特征
    df_with_features = create_static_features(df, target_col='Target')
    
    # 运行实验
    results = evaluate_on_multiple_models(df_with_features, target_col='Target')
